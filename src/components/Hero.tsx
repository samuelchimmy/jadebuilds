...
import useAdvancedTypingEffect from '../hooks/useAdvancedTypingEffect';

// The old useTypingEffect hook is no longer needed.
...
const Hero = () => {
  ...
  // === 2. USE THE NEW ADVANCED HOOK ===
  const { displayText, showCursor } = useAdvancedTypingEffect(
    "Hello! I'm Samuel", 
    {
      typeSpeed: 85,
  ...
  return <div className="min-h-screen text-foreground flex flex-col">
        ...
        <div className="fixed inset-0 bg-background z-50 flex flex-col items-center justify-center animate-fade-in">
          ...
          <nav>
            <ul className="flex flex-col items-center gap-8 text-center">
              <li><a onClick={() => handleMenuLinkClick('projects')} className="text-4xl font-bold text-muted-foreground hover:text-foreground transition-colors cursor-pointer">Projects</a></li>
              <li><a onClick={() => handleMenuLinkClick('contact')} className="text-4xl font-bold text-muted-foreground hover:text-foreground transition-colors cursor-pointer">Contact</a></li>
            </ul>
          </nav>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 pt-24">
        ...
        {/* === 3. UPDATE THE JSX TO DISPLAY THE NEW ANIMATION === */}
        ...
        <p className="text-center text-muted-foreground max-w-2xl mx-auto mb-6 leading-relaxed animate-fade-in text-sm sm:text-base px-4" style={{ animationDelay: '0.6s' }}>
          I'm a <strong className="text-foreground">Software Engineer</strong> with passion for problem-solving...
        </p>
        ...
        <div className="my-8 animate-fade-in" style={{ animationDelay: '1s' }}>
          <div className="flex justify-center space-x-6">
            <a href="https://github.com/samuelchimmy" aria-label="GitHub" className="text-muted-foreground hover:text-foreground transition-colors"><Github size={20} /></a>
            <a href="https://x.com/MetisCharter" aria-label="Twitter" className="text-muted-foreground hover:text-foreground transition-colors"><X size={20} /></a>
            <a href="https://www.linkedin.com/in/samuelchimmy1" aria-label="LinkedIn" className="text-muted-foreground hover:text-foreground transition-colors"><Linkedin size={20} /></a>
            <a href="https://link3.to/jadeofwallstreet" aria-label="More Links" className="text-muted-foreground hover:text-foreground transition-colors"><Link size={20} /></a>
          </div>
        </div>

        {showPortfolio && <div className="w-full max-w-4xl animate-fade-in space-y-8">{/*...rest of your project mapping code...*/}</div>}
      </div>
    </div>;
};
export default Hero;
