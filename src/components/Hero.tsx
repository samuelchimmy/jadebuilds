...
const Hero = () => {
  ...
  const handleExploreClick = () => {
    // This toggles the portfolio visibility, keeping its original function
  ...
  const handleMenuLinkClick = (sectionId: string) => {
    ...
    }, 150); // A slightly longer delay to be safe
  };
  ...
  return <div className="min-h-screen text-foreground flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-8 pt-24">
        ...
        {/* Explore button retains its original toggle functionality */}
        <button onClick={handleExploreClick} className="flex items-center gap-2 px-4 py-2 sm:px-6 sm:py-3 rounded-md bg-foreground text-background hover:bg-foreground/80 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:shadow-muted-foreground/30 animate-fade-in text-sm sm:text-base">
          {showPortfolio ? 'Hide Details' : 'Explore'} 
          <ExternalLink size={16} className="sm:w-[18px] sm:h-[18px]" />
        </button>
        <div className="my-8 animate-fade-in" style={{ animationDelay: '1s' }}>
          <div className="flex justify-center space-x-6">
            <a href="https://github.com/samuelchimmy" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" aria-label="GitHub"><Github size={20} className="sm:w-6 sm:h-6" /></a>
            <a href="https://x.com/MetisCharter" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" aria-label="Twitter"><X size={20} className="sm:w-6 sm:h-6" /></a>
            <a href="https://www.linkedin.com/in/samuelchimmy1" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" aria-label="LinkedIn"><Linkedin size={20} className="sm:w-6 sm:h-6" /></a>
            <a href="https://link3.to/jadeofwallstreet" target="_blank" rel="noopener noreferrer" className="text-muted-foreground hover:text-foreground transition-colors" aria-label="More Links"><Link size={20} className="sm:w-6 sm:h-6" /></a>
          </div>
        </div>
        ...
        {showPortfolio && <div className="w-full max-w-4xl animate-fade-in space-y-8">
            ...
            <div id="contact" className="bg-card rounded-lg p-6 border border-border scroll-mt-20">
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                <a href="mailto:EdozieSammy101@gmail.com" className="flex items-center gap-2 px-4 py-2 rounded-md bg-foreground text-background hover:bg-foreground/80 transition-all duration-300 text-center justify-center"><Mail size={18} /> Work With Me</a>
                <a href="https://flowcv.com/resume/d5baa23saiuu" target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 rounded-md border border-border text-foreground hover:bg-muted transition-all duration-300 text-center justify-center"><Download size={18} /> Download Resume</a>
              </div>
            </div>
                  ...
                  <div>
                    {item.tags && (
                      <div className="flex flex-wrap gap-2 mt-auto mb-4">
                        {item.tags.map(tag => ( <span key={tag} className="text-xs text-muted-foreground bg-muted px-2 py-1 rounded">{tag}</span> ))}
                      </div>
                    )}
                    {item.link && ( <a href={item.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-foreground hover:text-muted-foreground transition-colors text-sm font-medium">{item.title.includes("Blog") || item.title.includes("Docs") ? "Read More" : "Visit Site"} <ExternalLink size={14} /></a> )}
                    {item.links && ( <div className="flex gap-4">{item.links.map(link => ( <a key={link.name} href={link.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-foreground hover:text-muted-foreground transition-colors text-sm font-medium">{link.name} <ExternalLink size={14} /></a> ))}</div> )}
                  </div>
                </div>
              ))}
            </div>
